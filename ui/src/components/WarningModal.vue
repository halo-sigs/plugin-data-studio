...
<template>
  <VModal v-if="warningModalVisible" ref="modal" title="警告" :width="520" @close="emit('close')">
    <div>
      <span class=":uno: text-sm font-semibold">
      ...
      <span class=":uno: text-sm font-semibold">相关资料：</span>

      <br />

      <ul class=":uno: mt-1 text-sm space-y-1">
        <li>
          1.
          <a
    ...
    <template #footer>
      <div class=":uno: flex justify-between">
        ...
        <VButton type="secondary" @click="$router.push({ name: 'Backup' })">去备份</VButton>
      </div>
    </template>
  </VModal>
</template>
